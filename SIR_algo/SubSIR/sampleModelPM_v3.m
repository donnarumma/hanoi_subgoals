% function [state] = sampleModelPM_v3(model,oldstate,MC)
% 
% Authors:     Francesco Donnarumma, Giovanni Pezzulo, Domenico Maisto
%
% Description: lookahead sampling
% 
% Note:
...
%     fprintf('Processing Particle %g/%g...\n',MC.n,MC.N);
    ...
    if Stp1==SGtp1 && Stp1 ~= MC.goal_state
        ...
        parfor i=1:howmany
            ...
            wSG(i) = estimateNextSubgoal  ( MC.goal_state,       ...
                                            next_SG(i),          ...
                                            previous_SG,         ...
                                            subgoal_priors,   ...
                                            MC.transitions_cpt,  ...
...
%         end
    ...
    if w1 == 0
        w1 = (1/10^308.2547);
        if SGtp1 == Stp1
            fprintf(' compute_policy_prob = 0 | updatePi(S%g,S%g) [new]',SGtp1,next_SG);
        else
            fprintf(' compute_policy_prob = 0 | updatePi(S%g,S%g)',Stp1,SGtp1);
        end
    end
    if w2 == 0
       % avoide divide by zero
       w2 = (1/10^308.2547);
       fprintf(' estimate_next_subgoal p(SG%g|SG%g,S%g) = 0 |',next_SG, previous_SG,previous_S)
    end
    if warn
        fprintf('\n');
    end
